...
export const getCountriesWithCode = async (req: Request, res: Response) => {
  try {
    const repoCountry = AppDataSource.getRepository(CountryOrigin);
    const countryObj = (await repoCountry.findBy({})).sort((a, b) => {
      if (a.countryoriginid > b.countryoriginid) return 1;
    ...
    const countries: { country: string; code: string }[] = countryObj.map(
      (country) => {
        return { country: country.country, code: country.code };
      }
    );
